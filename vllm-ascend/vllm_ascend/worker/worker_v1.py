...
class NPUWorker(WorkerBase):
    ...
    def __init__(
        ...
        #logger.info(f"At the npu worker the vllm config is {self.vllm_config}")
    ...
    def init_device(self):
        ...
        #logger.info(f"Model runner class is {model_runner_class}")
    ...
    def execute_model(
        ...
        #logger.info("The model is execute here in the ascend worker")
        intermediate_tensors = None
        if not get_pp_group().is_first_rank:
            intermediate_tensors = IntermediateTensors(
                get_pp_group().recv_tensor_dict(
                    all_gather_group=get_tp_group()))
        #logger.info(f"Encoder cache before {self.model_runner.encoder_cache}")
        output = self.model_runner.execute_model(scheduler_output,
                                                 intermediate_tensors)
        #logger.info(f"Encoder cache after {self.model_runner.encoder_cache}")
    ...
    def initialize_from_config(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.vllm_config.model_config.enable_sleep_mode:
            ...
            context = nullcontext()  # type: ignore
        with context:
            #logger.info(f"KV Cache config is {kv_cache_config}")
