...
class NPUModelRunner(LoRAModelRunnerMixin):
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        #logger.info(f"Request data is {req_data}")
    ...
    def _execute_mm_encoder(self, scheduler_output: "SchedulerOutput"):
        ...
        #logger.info(f"At model runner v1 scheduled encoder inputs is {scheduled_encoder_inputs}")
        ...
        for req_id, encoder_input_ids in scheduled_encoder_inputs.items():
            req_state = self.requests[req_id]
            #logger.info(f"At model runner v1 req state is {req_state}")
        ...
        #logger.info(f"At model runner v1 encoder output is {encoder_outputs}")
        # Cache the encoder outputs.
        for (req_id, input_id, pos_info), output in zip(
                req_ids_pos,
                encoder_outputs,
        ):
            #logger.info(f"Collect encoder cache {req_id} {input_id} {pos_info}")
        ...
        #logger.info(f"Encoder cache is {self.encoder_cache}")
    ...
    def execute_model(
        ...
        #logger.info("This should not be run")
        ...
        with self.maybe_dummy_run_with_lora(self.lora_config,
            ...
            with set_ascend_forward_context(
                ...
                if self.torchair_graph_enabled and not with_prefill:
                    ...
                    maybe_converting_weight_acl_format(self.model,
                                                       ACL_FORMAT_FRACTAL_ND)
                    # logger.info(f"Input ids device is {input_ids.device}")
                    # logger.info(f"positions ids device is {positions.device}")
                    # logger.info(f"inputs_embeds ids device is {inputs_embeds.device}")
                    # logger.info(f"Model device is {model.language_model.model.layers[0].input_layernorm.weight.device}")
                    positions = positions.to(torch_npu.npu.current_device())
                    inputs_embeds = inputs_embeds.to(torch_npu.npu.current_device())
                    hidden_states = model(
                        input_ids=input_ids,
                        positions=positions,
                        intermediate_tensors=intermediate_tensors,
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        """
        #logger.info(f"At init kvc the device is {self.device}")
